...
export default function SidebarLink({ link, iconName }) {
  ...
  return (
    ...
    <NavLink to = {link.path} onClick = {() => dispatch(resetCourseState())}
             className = {`relative px-8 py-2 text-sm font-medium ${matchRoute(link.path) ? "bg-indigo-600 text-yellow-50" : "bg-opacity-0 text-richblack-300" } transition-all duration-200`} >
